...
void handle_instruction(Instruction_t *instruction, int tick) {
	...
	if(instruction->burst_time == 0) { 
        ...
        task->id = task_id;
		task->current_queue = 1;         //Initialize the tasks' queue to 1
		...
		// TODO ... Insert your Code Here
        //Find the waiting time and turn around time
        ...
        enqueue(get_queue_by_id(task->current_queue), task);    //enqueue the current task
	}
}
...
Task_t *peek_priority_task() {
	// TODO ... Insert your Code Here
    //Checks if all the queues are not empty, otherwise return NULL
...
void boost(int tick) {
    ...
    Task_t *task;
    //If current task exists, set to queue 1 and check its remaining quantum time.
    if (current_task != NULL && remaining_quantum > 2){
            remaining_quantum = 2;
        }

    //Place all/boosts tasks from queue 3 on queue 1
    ...
    //Place all/boosts tasks from queue 2 onto queue 1
...
void scheduler() {
    ...
    if (current_task != NULL && remaining_quantum == 0) {
        ...
        //The task needs to be moved into the next queue or it is finished
        if (current_task->remaining_burst_time > 0) {
            decrease_task_level(current_task);
            enqueue(get_queue_by_id(current_task->current_queue), current_task);       //enqueue using the queue of the current task and id
        }
        current_task = NULL;     //make the current null since no more quantum time left
    } 
    ...
    if (priority_task != NULL && current_task == NULL){
        ...
        remaining_quantum = QUEUE_TIME_QUANTUMS[current_task->current_queue - 1];   //Find the remining quantum time, queueID is offset by 1 to queue index    
    }

    else if (priority_task != NULL && current_task->current_queue > priority_task->current_queue){
        enqueue(get_queue_by_id(current_task->current_queue), current_task);
        current_task = dequeue(get_queue_by_id(priority_task->current_queue));
        remaining_quantum = QUEUE_TIME_QUANTUMS[current_task->current_queue - 1];     //Find the remining quantum time
    }
}
...
void update_task_metrics() {
    ...
    for(int i = 0; i < MAX_TASKS; i++) {
        ...
        if(task->remaining_burst_time != 0 && task->remaining_burst_time > 0 && task != current_task){
            //Updates the variables of tasks that aren't the current task
