...
void kosmos_init() {
    ...
    sem_init(&oxygen, 0, 0);
    sem_init(&bonding, 0, 1);
}

void check_num_atoms(char* name){
    //Creating the radical
    sem_wait(&bonding);
    if(freeCarbons >= 1 && freeHydrogens >= 3 && freeOxygens >= 1){
        ...
        sem_post(&mutex); //resets mutex back to 1
    }
    sem_post(&bonding);
    return;
}
...
void *h_ready( void *arg )
{
    ...
    freeHydrogens++;
    hydrogenID[hydroIndex] = id; //stores 3 hydrogen ID's
    hydroIndex = (hydroIndex + 1) % 3;

    //Creates 3 hydrogens before making other atoms
    if((freeHydrogens % 3) == 0){
        sem_post(&carbon);
        sem_post(&oxygen);
        sem_wait(&hydrogen); //suspends since we don't need more
    }
    else{
        sem_post(&mutex); //allows another hydrogen to be created
        sem_wait(&hydrogen);
    }
	return NULL;
}
